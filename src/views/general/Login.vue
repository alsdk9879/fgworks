...
.box-wrap
    ...
    template(v-if="route.query.support")
        ...
        form(@submit="login")
            ...
            label
                input(type="checkbox" @change="changeRemember" :checked="remember")
                span 아이디 저장
...
const remember = ref(window.localStorage.getItem('remember') === 'true');
...
let changeRemember = (e) => {
    if (e.target.checked) {
        remember.value = true;
    } else {
        remember.value = false;
    }
    window.localStorage.setItem('remember', remember.value ? 'true' : 'false');
}
let login = (e) => {
